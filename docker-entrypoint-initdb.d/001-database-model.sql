...
CREATE TABLE IF NOT EXISTS `demodb`.`Products` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `id_idx` (`ProductsCategories_id` ASC) VISIBLE,
  ...
  CONSTRAINT `Products_ProductsCategories_id`
...
CREATE TABLE IF NOT EXISTS `demodb`.`ProductsPrices` (
  ...
  INDEX `product_date_index` (`Products_id` ASC, `date_start` DESC) VISIBLE,
  CONSTRAINT `ProductsPrices_Products_id`
...
CREATE TABLE IF NOT EXISTS `demodb`.`ProductsRatings` (
  ...
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `Products_id_idx` (`Products_id` ASC) VISIBLE,
  INDEX `product_date_index` (`Products_id` ASC, `date` DESC) VISIBLE,
  INDEX `rating_index` (`rating` DESC) VISIBLE,
  CONSTRAINT `ProductsRatings_Products_id`
    FOREIGN KEY (`Products_id`)
    REFERENCES `demodb`.`Products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `demodb`.`ProductsInventory` (
  ...
  `TransactionProduct_id` INT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `Products_id_idx` (`Products_id` ASC) VISIBLE,
  INDEX `date_index` (`date` DESC) VISIBLE,
  INDEX `product_date_index` (`Products_id` ASC, `date` DESC) VISIBLE,
  CONSTRAINT `ProductsInventory_Products_id`
    FOREIGN KEY (`Products_id`)
    REFERENCES `demodb`.`Products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- Table `demodb`.`Roles`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `demodb`.`Roles` ;

CREATE TABLE IF NOT EXISTS `demodb`.`Roles` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `demodb`.`UsersRoles` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uq_User_Role` (`Users_id` ASC, `Roles_id` ASC) VISIBLE,
  INDEX `fk_Roles_1_idx` (`Users_id` ASC) VISIBLE,
  INDEX `fk_Roles_2_idx` (`Roles_id` ASC) VISIBLE,
...
BEGIN
START TRANSACTION;
	INSERT INTO Products(code, name, description, ProductsCategories_id, image, deleted) 
		VALUES(_code, _name, _description, _category, _image, 0);
	SET @newid = LAST_INSERT_ID();
	INSERT INTO ProductsPrices (Products_id, price) 
		VALUES(@newid, _price);
	INSERT INTO ProductsInventory (Products_id, quantity) 
		VALUES (@newid, _quantity);
	INSERT INTO ProductsRatings (Products_id, rating_count_1, rating_count_2, rating_count_3, rating_count_4, rating_count_5 ) 
...
START TRANSACTION;
	IF (coalesce(_code,_name,_description, _image, _category) is not null) THEN BEGIN
		UPDATE Products p SET 
			...
			p.ProductsCategories_id = COALESCE(_category, p.ProductsCategories_id)
		WHERE p.id = _id;
    END;
    END IF;
    IF (_price is not NULL) THEN BEGIN 
    SET @now = NOW();
	UPDATE ProductsPrices SET date_end = @now WHERE date_start <= @now and date_end IS NULL and Products_id = _id;
    INSERT INTO ProductsPrices (Products_id, date_start, price) VALUES (_id, @now, _price); 
    END; END IF;
    IF (_quantity is not NULL) THEN BEGIN INSERT INTO ProductsInventory (Products_id, quantity) VALUES (_id, _quantity); END; END IF;
COMMIT;
END$$
